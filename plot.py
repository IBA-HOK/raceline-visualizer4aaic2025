...
class RaceLineEditApp:
    def __init__(self, root):
        ...
        self.raceline_scatter = None;
        self.raceline_line = None
    ...
    def setup_left_panel(self):
        ...
        file_frame = ttk.LabelFrame(left_frame, text="ファイル操作", padding="10");
        ...
        self.tree = ttk.Treeview(left_frame, columns=('Index', 'X', 'Y', 'Speed'), show='headings',
                                 selectmode='extended');
        self.tree.heading('Index', text='点 #');
        ...
        edit_frame = ttk.LabelFrame(left_frame, text="選択した点を編集", padding="10");
        ...
        ttk.Label(edit_frame, text="速度:").grid(row=2, column=0, sticky=tk.W, pady=2);
        ttk.Entry(edit_frame, textvariable=self.speed_var).grid(row=2, column=1, sticky=tk.EW)
        ttk.Button(edit_frame, text="値を更新してプロット再描画", command=self.update_values_from_entry).grid(row=3, column=0,
                                                                                                       columnspan=2,
                                                                                                       pady=10)
    ...
    def update_plot(self):
        ...
        if self.df is not None and not self.df.empty:
            ...
            self.raceline_scatter = ax.scatter(data[:, 0], data[:, 1], c=data[:, 2], cmap='jet', s=25,
                                               label='racing line(point)', picker=5, zorder=10)
            line_plot_data = np.vstack([data[:, :2], data[0, :2]])
            self.raceline_line, = ax.plot(line_plot_data[:, 0], line_plot_data[:, 1], color='cyan', linewidth=1.2,
                                          label='racing line (line)', zorder=5)
            ####
            self.highlight_scatter, = ax.plot([], [], 'o', markersize=10, markerfacecolor='none',
                                              markeredgecolor='yellow', markeredgewidth=2, zorder=20)
            ####
    ...
    def update_values_from_entry(self):
        ...
        try:
            ...
            messagebox.showerror("エラー", "速度には数値を入力してください。")
    ...
    def load_osm_map(self):
        ...
        try:
            ...
            messagebox.showinfo("成功", f"{len(self.map_nodes)}個の地図ノードを読み込みました。");
            self.update_plot()
        except Exception as e:
    ...
    def save_csv(self):
        ...
        try:
            file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")],
                                                     title="名前を付けて保存", initialfile="raceline_modified.csv")
            if file_path:
    ...
    def load_csv(self):
        ...
        try:
            ...
            rename_map = {'# x_m': 'x', 'x_m': 'x', 'y_m': 'y', 'vx_mps': 'speed'};
            self.df.rename(columns=rename_map, inplace=True)
            if 'x' not in self.df.columns or 'y' not in self.df.columns or 'speed' not in self.df.columns: messagebox.showerror(
                "エラー", "CSVに 'x', 'y', 'speed' のいずれかの列が含まれていません。"); self.df = None; return
            self.history = []
            self.populate_table();
            self.update_plot()
    ...
    def populate_table(self):
        ...
        for index, row in self.df.iterrows():
            self.tree.insert("", "end", iid=str(index),
                             values=(index, f"{row['x']:.2f}", f"{row['y']:.2f}", f"{row['speed']:.2f}"))
